...
function UploadTab() {
  ...
  const { presentationOptions, setPresentationOptions } = useImageToSlide();
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files) {
      ...
      const uploadedFileUrls = [
        ...presentationOptions[EGeneratedSlideForm.FILE_URLS],
        ...fileUrls,
      ];
      setPresentationOptions({
        ...presentationOptions,
        [EGeneratedSlideForm.CONTENT]: uploadedFiles,
        [EGeneratedSlideForm.FILE_URLS]: uploadedFileUrls,
      });
  ...
  const handleRemoveUploadedFile = (url: string) => {
    const fileIndex =
      presentationOptions[EGeneratedSlideForm.FILE_URLS].indexOf(url);
    const updatedFiles = presentationOptions[
      EGeneratedSlideForm.CONTENT
    ].filter((_, index) => index !== fileIndex);
    const updatedFileUrls = presentationOptions[
      EGeneratedSlideForm.FILE_URLS
    ].filter((fileUrl) => fileUrl !== url);

    setPresentationOptions({
      ...presentationOptions,
      [EGeneratedSlideForm.CONTENT]: updatedFiles,
      [EGeneratedSlideForm.FILE_URLS]: updatedFileUrls,
    });
  };
    ...
    <div className="flex flex-col items-center justify-center w-full">
      ...
      <label
        ...
        className={`flex flex-col items-center justify-center w-full mb-4 h-64 p-4 xl:p-0 border-2 border-purple-300 border-dashed rounded-lg cursor-pointer bg-purple-50 dark:bg-gray-700 hover:bg-purple-100 dark:border-purple-600 dark:hover:border-purple-500 dark:hover:bg-purple-600 ${
          presentationOptions[EGeneratedSlideForm.FILE_URLS].length > 0
            ? "h-20"
            : ""
        }`}
      >
        <div className="flex flex-col items-center justify-center pt-5 pb-6">
          ...
          <p className="text-center mb-2 text-sm text-purple-500 dark:text-purple-400">
      ...
      {presentationOptions[EGeneratedSlideForm.FILE_URLS].length > 0 && (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 mb-4">
          {presentationOptions[EGeneratedSlideForm.FILE_URLS].map(
            (file, index) => (

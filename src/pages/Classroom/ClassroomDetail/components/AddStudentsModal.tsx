...
function AddStudentModal({ open, setOpen }: AddStudentModalProps) {
  ...
  const handleSearch = async (query: string) => {
    if (!query.trim()) {
      setFilteredUsers([]); // Nếu không có từ khóa, không hiển thị danh sách
      return;
    }
    const response = users.filter((user) =>
      user.fullName.toLowerCase().includes(query.toLowerCase())
    );
    setFilteredUsers(response); // Lưu kết quả tìm kiếm
  };
  ...
  const handleAddStudents = () => {
    ...
    console.log("Students added:", selectedUsers);
